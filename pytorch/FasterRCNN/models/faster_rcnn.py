...
class FasterRCNNModel(nn.Module):
  ...
  def train_step(self, optimizer, image_data, anchor_map, anchor_valid_map, gt_rpn_map, gt_rpn_object_indices, gt_rpn_background_indices, gt_boxes):
    ...
    # Stage 3: Detector
    # We add two more attributes for the adversarial example, one for the class
    # and the other for the bbox
    detector_classes, detector_box_deltas, detector_classes_noise, detector_box_deltas_noise = self._stage3_detector_network(
      feature_map = feature_map,
      proposals = proposals
    )
